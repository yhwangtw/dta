...
- **誠實的狀態** — 執行中有轉圈 + 已耗時計時;模型停止回應時顯示停滯警告;失敗的執行以紅色錯誤卡顯示完整錯誤原因(絕不無聲失敗)
...
- **⌘F 對話內搜尋** — 比對計數、Enter/⇧Enter 循環、目標閃光標示
- **圖示導航欄** — 對話 / 檔案 / 變更 / 搜尋 / 分析視圖;底部有模型 / 技能 / 語言 / 主題
- **跳到底部** — 上捲時出現浮動按鈕;捲到尾端會啟用串流黏底跟隨
...
### 外觀
- **四套可切換外觀** — Editorial 米紙暖色(預設)/ Terminal 翡翠綠 / Industrial 工業黑白 / Aurora 極光紫,每套都有明暗主題。`⌘K` → "Appearance" 切換
